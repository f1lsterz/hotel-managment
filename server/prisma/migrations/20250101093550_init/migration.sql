...
CREATE TABLE `User` (
    ...
    `role` ENUM('ADMIN', 'MANAGER', 'STAFF', 'USER', 'GUEST') NOT NULL,
...
CREATE TABLE `Payment` (
    ...
    `method` ENUM('CREDIT_CARD', 'PAYPAL', 'STRIPE') NOT NULL,
...
CREATE TABLE `Report` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `type` ENUM('FINANCIAL', 'OCCUPANCY', 'GENERAL') NOT NULL,
...
CREATE TABLE `_BookingToService` (
    ...
    UNIQUE INDEX `_BookingToService_AB_unique`(`A`, `B`),
    INDEX `_BookingToService_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
